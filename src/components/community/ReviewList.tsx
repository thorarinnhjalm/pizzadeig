...
export function ReviewList({ targetId, targetType, locale }: Props) {
  ...
  if (reviews.length === 0) {
    return <p className="text-muted-foreground italic py-8 bg-muted rounded-2xl text-center border border-dashed border-border">{locale === 'is' ? 'Engar umsagnir ennþá. Vertu fyrstur!' : 'No reviews yet. Be the first!'}</p>;
  }

  return (
    <div className="space-y-6 mt-8">
      <h3 className="font-bold text-2xl text-foreground font-display">{locale === 'is' ? 'Umsagnir' : 'Reviews'}</h3>
        ...
        {reviews.map((review) => {
          ...
          return (
            <div key={review.id} className="bg-card p-6 rounded-2xl border border-border shadow-sm">
              ...
              {comment && <p className="text-muted-foreground text-sm mt-3 mb-4 leading-relaxed">{comment}</p>}
              <div className="flex items-center mt-2 pt-4 border-t border-border">
