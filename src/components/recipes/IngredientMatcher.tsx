...
export function IngredientMatcher({ locale }: Props) {
  ...
  return (
    <div className="space-y-12">
      {/* Ingredient selector */}
      <div className="bg-secondary rounded-2xl border border-border p-6 md:p-8 shadow-md">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-muted text-foreground border border-border/50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary"
          />
        ...
        {selected.size > 0 && (
          <p className="text-sm text-muted-foreground mt-4">
            ...
            <button onClick={() => setSelected(new Set())} className="ml-3 text-primary font-bold hover:underline cursor-pointer">
            ...
            {matchedRecipes.map(({ recipe, score, matchCount }) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-secondary border border-border rounded-xl overflow-hidden hover:shadow-xl transition-all hover:-translate-y-1 flex"
                >
                  <div className="w-32 h-32 relative shrink-0 bg-muted">
                    {recipe.image_urls?.[0] && (
                      <Image src={recipe.image_urls[0]} alt={title} fill className="object-cover" />
                    )}
                  </div>
                  <div className="p-4 flex-1 min-w-0">
                    <h4 className="font-bold text-foreground group-hover:text-primary truncate">{title}</h4>
