...
export function RecipeCard({ recipe, locale }: RecipeCardProps) {
  ...
  return (
    <Link href={`/uppskriftir/${recipe.slug}`} className="group block h-full">
      <Card className="h-full border-(--color-border) border-b-[3px] border-b-(--color-gold) hover:border-b-(--color-brand) card-hover flex flex-col bg-(--color-bg-secondary) overflow-hidden transition-all duration-300">
        ...
        <CardContent className="p-5 flex flex-col flex-1 relative pt-6">
          <Badge className="absolute -top-3 left-5 bg-(--color-terracotta) text-(--color-text-primary) hover:bg-(--color-terracotta) font-chalk text-[0.95rem] tracking-wide px-3 py-1 shadow-sm border-none">
            {recipe.category || recipe.type}
          </Badge>
          <h3 className="font-display font-bold text-xl mb-2 text-(--color-text-primary) group-hover:text-(--color-brand) transition-colors line-clamp-2">{title}</h3>
          <p className="text-(--color-text-secondary) text-sm mb-4 line-clamp-2 font-body">{description}</p>
          
          <div className="mt-auto flex items-center justify-between text-[11px] text-muted-foreground uppercase tracking-wider font-bold pt-4 border-t border-(--color-border)">
            <span className="flex items-center">
              <Flame className="w-3.5 h-3.5 mr-1 text-(--color-brand)" /> 
            ...
            <span className="flex items-center text-(--color-text-secondary) font-medium bg-(--color-bg-tertiary) px-2 py-1 rounded">
              <Clock className="w-3.5 h-3.5 mr-1 text-ring" /> {formatTime(totalTime)}
            </span>
          </div>
        </CardContent>
      </Card>
